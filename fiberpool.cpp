...
struct SomeFunctor {
  ...
  void operator()(std::string const &msg) {
    ...
    while (++i < 5) {
      ...
      boost::this_fiber::sleep_for(std::chrono::seconds(sleep_time));
...
void find_primes(uint64_t n, std::vector<uint64_t> &primes) {
...
void custom_pool() {
  ...
  auto primes_future = fiber_pool.submit(&find_primes, 20, std::ref(primes));

  // wait for factorial task to finish
  auto factorial_calculated = factorial_future->get();

  std::cout << "Factorial calcualted: " << factorial_calculated << std::endl;
...
int main(int argc, const char *argv[]) {
  ...
  for (auto i = 0; i < 10; i++) {
    ...
    // has switch thread
    auto &&a_future = DefaultFiberPool::submit_job(
        SomeFunctor(), "functor: " + std::to_string(i));
  ...
  auto future_3 =
      DefaultFiberPool::submit_job(boost::fibers::launch::dispatch, throws);
  ...
  if (exp_ptr) {
    ...
    try {
      ...
      std::cout << "catched rethrown exception: " << e.what() << std::endl;
    }
  }
